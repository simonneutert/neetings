...
export function sortBySortKey<T extends { sortKey: string }>(items: T[]): T[] {
  // Use direct string comparison (charCode ordering) so that uppercase 'Z' keys
  // generated by generateKeyBefore() sort correctly before lowercase 'a'.
  // localeCompare() is locale-aware and reverses this order in most locales.
