...
describe("sortKeys utilities", () => {
  ...
  describe("sortBySortKey", () => {
    ...
    it("sorts uppercase Z keys before lowercase keys (charCode order)", () => {
      // generateKeyBefore("a") produces "Z", which must sort BEFORE "a".
      // This is a regression test: localeCompare() placed "Z" after "a" in most
      // locales, causing blocks moved to the first position to render last.
      const items = [
        { id: "3", sortKey: "aa" },
        { id: "2", sortKey: "a" },
        { id: "1", sortKey: "Z" }, // "before-first" key generated by generateKeyBefore("a")
      ];

      const sorted = sortBySortKey(items);

      expect(sorted.map((item) => item.id)).toEqual(["1", "2", "3"]);
      expect(sorted[0].sortKey).toBe("Z");
      expect(sorted[1].sortKey).toBe("a");
      expect(sorted[2].sortKey).toBe("aa");
    });
